...
dssV2.parser( 'name', function () {
  return this.line.contents;
});

// Describe default parsing of a description
dssV2.parser( 'description', function () {
  return this.line.contents;
});

// Describe default parsing of a state
dssV2.parser('state', function () {
var state = this.line.contents.split(' - ');
return [{
  name: state[0] ? dssV2.trim(state[0]) : '',
  escaped: state[0] ? dssV2.trim(state[0].replace('.', ' ').replace(':', ' pseudo-class-')) : '',
  description: state[1] ? dssV2.trim(state[1]) : ''
}];
});

// Describe default parsing of a prop
dssV2.parser('prop', function () {
var prop = this.line.contents.split(' - ');
return [{
  name: prop[0] ? dssV2.trim(prop[0]) : '',
  escaped: prop[0] ? dssV2.trim(prop[0].replace('.', ' ').replace(':', ' pseudo-class-')) : '',
  description: prop[1] ? dssV2.trim(prop[1]) : ''
}];
});

// Describe default parsing of a param
dssV2.parser('param', function () {
var param = this.line.contents.split(' - ');
return [{
  name: param[0] ? dssV2.trim(param[0]) : '',
  escaped: param[0] ? dssV2.trim(param[0].replace('.', ' ').replace(':', ' pseudo-class-')) : '',
  description: param[1] ? dssV2.trim(param[1]) : ''
}];
});

// Describe default parsing of a piece markup
dssV2.parser( 'markup', function () {
  return [{
...
dssV2.parser('category', function() {
  return this.line.contents;
});
dssV2.parser('component', function() {
  return this.line.contents;
});
dssV2.parser('section', function() {
  return this.line.contents;
});
dssV2.parser('variation', function() {
  return this.line.contents;
});

dssV2.parser('hidecode', function() {
  return this.line.contents;
});

dssV2.parser('html', function() {
  return [{
    example: this.line.contents,
    escaped: this.line.contents.replace(/</g, '&lt;').replace(/>/g, '&gt;')
  }];
...
dssV2.parser('variable', function() {
  var variable = this.line.contents.split(':');
  return [{
    name: (variable[0]) ? dssV2.trim(variable[0]) : '',
    escaped: (variable[0]) ? dssV2.trim(variable[0].replace('.', ' ').replace(':', ' pseudo-class-')) : '',
    description: (variable[1]) ? dssV2.trim(variable[1]) : ''
  }];
});
